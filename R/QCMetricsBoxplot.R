...
#' @param entity_type Character string. The name of the metadata column to group cells by
#'   (e.g., "orig.ident", "batch", "seurat_clusters").
#' @param entity_name Character string or NULL. If specified, filters the plot to show only
...
#' @param gradient_legend Boolean. Whether to show legend for gradient coloring. Default is FALSE.
#'
...
QCMetricsBoxplot <- function(
  ...
  # Check if entity_type exists in metadata
  if (!entity_type %in% colnames(SeuratObject@meta.data)) {
    stop(paste0(
      "'entity_type' column '",
      entity_type,
  ...
  if (length(missing_metrics) > 0) {
    stop(paste0(
      "The following QC metrics not found in meta.data: ",
  ...
  # Filter to entity_name if specified
  if (!is.null(entity_name)) {
    if (!entity_name %in% metadata[[entity_type]]) {
      stop(paste0(
        "'entity_name' value '",
        entity_name,
        "' not found in '",
    ...
    metadata <- metadata[metadata[[entity_type]] == entity_name, , drop = FALSE]
  }
  ...
  for (metric in qc_metrics) {
    ...
    # Calculate outilers for boxplot (using IQR method).
    Q1 <- quantile(plot_data$value, 0.25)
    Q3 <- quantile(plot_data$value, 0.75)
    IQR <- Q3 - Q1
    lower_bound <- Q1 - 1.5 * IQR
    upper_bound <- Q3 + 1.5 * IQR
    plot_data$outlier <- (plot_data$value < lower_bound) |
      (plot_data$value > upper_bound)
    ...
    p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = entity, y = value)) +
      ...
      ggplot2::geom_boxplot(
        ...
        color = "black",
        alpha = 0.7,
        outlier.shape = NA, # Hide outliers since we will plot them as points
      ) +

      # Outliers with gradient coloring
      ggplot2::geom_jitter(
        ...
        size = pt.size,
        alpha = pt.alpha,
        show.legend = FALSE # Show no legend for gradient,
      ) +
